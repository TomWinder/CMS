...
EXPORT void scan4d(double *sigPt, int32_t *indPt, double *mapPt, int32_t fsmp, int32_t lsmp, int32_t nsamp, int32_t nstation, int64_t ncell)
{
	double	 *stnPt, *stkPt;
	int32_t  *ttpPt;
	int32_t  ttp, tend;
	...
	/* stack data.... */
	to      = MAX(0,MIN(fsmp,nsamp - 1));
	lsmp    = MAX(to,MIN(lsmp,nsamp));

	#pragma omp parallel for private(cell,tm,st,stnPt,stkPt,ttpPt,ttp,tend) /* shared(mapPt) */
	for (cell=0; cell<ncell; cell++)
	{
		stkPt = &mapPt[cell * (int64_t) nsamp + (int64_t) to];
		...
		{
			ttp     = MAX(0,ttpPt[st]);
			stnPt   = &sigPt[st*nsamp + ttp + to];
			tend    = MIN(lsmp,nsamp-ttp) - to;
			for(tm=0; tm<tend; tm++)
				stkPt[tm] += stnPt[tm];
		}
	}
...
EXPORT void detect4d(double *mapPt, double *snrPt, int64_t *indPt, int32_t fsmp, int32_t lsmp, int32_t nsamp, int64_t ncell)
{
	double	   mv, cv;
	int32_t  tm;
	int64_t  cell, ix;
...
EXPORT void detect4d_t(double *mapPt, double *snrPt, int64_t *indPt, int32_t fsmp, int32_t lsmp, int32_t nsamp, int64_t ncell)
{
	double	   *rPt, mv, cv;
	int32_t  tm;
	int64_t  cell, ix;
